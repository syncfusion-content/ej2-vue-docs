...
State persistence allows the Tab to retain the current model value in the browser cookies for state maintenance. This action is handled through the [enablePersistence](https://ej2.syncfusion.com/vue/documentation/api/tab#enablepersistence) property, which is set to `false` by default.

When it is set to `true`, some of the Tab component model values are retained even after refreshing the page.
