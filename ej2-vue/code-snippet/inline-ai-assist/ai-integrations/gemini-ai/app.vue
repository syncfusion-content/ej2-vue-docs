    ...
    mounted() {
        ...
        this.inlineAssist = new InlineAIAssist({
            ...
            inlineToolbarSettings: {
                itemClick: (args) => { if (args.item.iconCss === 'e-icons e-inline-stop') this.handleStopResponse(); }
            },
            responseSettings: {
                itemSelect: (args) => {
        ...
        if (summarizeBtn) {
            summarizeBtn.addEventListener('click', () => { this.inlineAssist.showPopup(); });
        }
    },
    beforeUnmount() {
    ...
    methods: {
        ...
        async onPromptRequest(args) {
            try {
                ...
                this.inlineAssist.addResponse('⚠️ Something went wrong while connecting to the AI service. Please check your API key or try again later.');
                stopStreaming = true;
            }
        },
