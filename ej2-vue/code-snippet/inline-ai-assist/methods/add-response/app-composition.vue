...
onMounted(() => {
    ...
    if (addResponseBtn) {
        addResponseBtn.addEventListener('click', addResponseHandler);
    }
});

onBeforeUnmount(() => {
    const addResponseBtn = document.querySelector('#addResponse');
    if (addResponseBtn && addResponseHandler) {
        addResponseBtn.removeEventListener('click', addResponseHandler);
