<template>
    <div class="control-section">
      <div id='flat-list'>
        <!-- ListView element -->
        <ejs-listview id='listview' ref='list' :dataSource='data' :fields='fields' headerTitle='Folders' showHeader='true'
          showIcon='true' :select='onSelect'>
        </ejs-listview>
      </div>
    </div>
  </template>
  ...
  const onSelect = (args) => {
    list.value.dataSource[args.index].child.push({ id: '01-02', text: 'Newly Added File', icon: 'file', htmlAttributes: { role: 'li', class: 'list' } });
  }
