      ...
      <ejs-listview id='ui-list' ref="list" :dataSource='listData' :showHeader='header' :fields='fields'
        ...
        <template v-slot:gTemplate="{ data }">
          <div class="e-list-wrapper e-list-avatar">
            <span :class="['e-avatar e-avatar-circle ' + data.icon + ' showUI']" v-if="data.imgUrl == undefined">{{
              data.icon }}</span>
            <span :class="['e-avatar e-avatar-circle ' + data.icon + ' hideUI']" v-if="data.imgUrl !== undefined">{{
              data.icon }}</span>
