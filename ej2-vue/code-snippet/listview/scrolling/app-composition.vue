  ...
  <div class="control-section">
    <div class="grid-container">
      <div>
        <h3>Chat</h3>
        <!-- ListView element -->
        <ejs-listview id='List' ref='list' :dataSource='visibleData' cssClass='e-list-template' :template='template'
          width='400px' height='320px' :scroll="onListScrolled"></ejs-listview>
      </div>
    </div>
  </div>
</template>
...
watch(
  data,
  (newVal) => {
    visibleData = newVal.slice(0, 5);
    remainingData = newVal.slice(5);
  }
...
const onListScrolled = (args) => {
  if (args.scrollDirection === "Bottom" && args.distanceY < 100) {
    const additionalItems = remainingData.slice(0, 5);
    visibleData = visibleData.concat(additionalItems);
