...
const getMongoClick = function() {
    dialog.value.content = querybuilder.value.getMongoQuery(querybuilder.value.getRules());
    dialog.value.show();
};

const getRuleClick = function() {
    var validRule = querybuilder.value.getValidRules(querybuilder.value.rule);
