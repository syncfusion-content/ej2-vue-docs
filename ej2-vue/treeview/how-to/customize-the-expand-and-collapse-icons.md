---
layout: post
title: customize expand and collapse icons in Vue TreeView | Syncfusion
description: Replace default Vue TreeView expand and collapse arrow icons with custom icons using the expandIcon and collapseIcon CSS class properties.
control: Customize the expand and collapse icons 
platform: ej2-vue
documentation: ug
domainurl: ##DomainURL##
---

# How to customize expand and collapse icons in Vue TreeView

To learn how to customize the appearance of the Vue TreeView component, watch this video:

{% youtube "https://www.youtube.com/watch?v=04dMG7EbVKU" %}

You can customize the TreeView expand and collapse icons by using the [cssClass](https://ej2.syncfusion.com/vue/documentation/api/treeview#cssclass) property of the TreeView.

Refer to the following sample to see how to customize the expand and collapse icons:

{% tabs %}
{% highlight html tabtitle="Composition API (~/src/App.vue)" %}
{% include code-snippet/treeview/how-to/icon-cs1/app-composition.vue %}
{% endhighlight %}
{% highlight html tabtitle="Options API (~/src/App.vue)" %}
{% include code-snippet/treeview/how-to/icon-cs1/app.vue %}
{% endhighlight %}
{% endtabs %}
        
{% previewsample "page.domainurl/code-snippet/treeview/how-to/icon-cs1" %}