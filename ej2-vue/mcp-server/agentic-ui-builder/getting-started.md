...
description: Learn how to set up and use the Syncfusion Vue UI Builder MCP Server for AI-powered assistance in building Vue applications with Syncfusion components.
control: Getting started with Syncfusion Vue UI Builder MCP Server
platform: ej2-vue
documentation: ug
domainurl: ##DomainURL##
---
...
The **Syncfusion<sup style="font-size:70%">&reg;</sup> Vue UI Builder** uses AI to help you build Vue applications with natural language commands. Simply describe what you want to create, and it generates complete UI implementations with Syncfusion components.
...

{
  "servers": {
    "sf-vue-mcp": {
      "type": "stdio",
      "command": "npx",
      "args": ["-y", "@syncfusion/vue-assistant@latest"],
      "env": {
...
**Verify Installation:** Check your editor's MCP Server list for `sf-vue-mcp` with "Connected" status to confirm successful installation.

## Usage

Once installed, open your AI assistant in the IDE and describe what you want to build with the `#sf_vue_ui_builder` command:

```
#sf_vue_ui_builder Create a dashboard with a sales data grid and monthly trend chart.
```

The UI Builder generates complete implementations including components and styling.

> Note: Using the `#sf_vue_ui_builder` tool ensures the Agentic UI Builder is invoked directly. Alternatively, you can use natural language without the tool—just make sure to include the "Syncfusion" keyword in your prompt so the AI model can automatically recognize and call the appropriate generator.
...
### Component Tool (`#sf_vue_component`)

Provides quick reference guidelines for Syncfusion Vue components, including properties, event handlers, methods, and usage examples.

**When to use:** To get basic component API information and structure details for implementing specific components correctly.

**Example:**
```
#sf_vue_component How do I integrate a data table?
```

### Style Tool (`#sf_vue_style`)

Provides theme configuration, styling setup, and icon integration for Syncfusion Vue components. Supports multiple themes (Tailwind3 CSS, Bootstrap 5.3, Material 3, Fluent 2), light/dark mode implementation, and icon patterns for consistent UI styling.

**When to use:** To apply themes, customize colors, modify the visual design of components, or integrate icons into your UI.

**Example:**
```
#sf_vue_style How do I apply the Syncfusion Tailwind 3 dark theme and add a communication icon inside a button?
```
...
- **Be specific:** Provide clear, specific descriptions of your component behavior and design preferences.
- **Reference patterns:** Mention existing design systems, component libraries, or specific patterns you want to replicate.
- **Stay consistent:** Maintain consistent file organization, naming conventions, and coding standards throughout your Vue project.
...
* **[Showcase Sample Projects](https://www.syncfusion.com/showcase-apps/vue)** - Full application examples
