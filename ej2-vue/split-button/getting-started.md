...
This article provides a step-by-step guide for setting up a Vue 2 project using [Vue-CLI](https://cli.vuejs.org) and integrating the Syncfusion<sup style="font-size:70%">&reg;</sup> Vue Split button component.
...
This article uses the [Vue Split button component](https://www.syncfusion.com/vue-components/vue-split-button) as an example. Install the `SplitButton` package by running the following command:
...
In this article, the `Material3` theme is applied using CSS styles, which are available in installed packages. The necessary `Material3` CSS styles for the Split button component and its dependents were imported into the `<style>` section of the **src/App.vue** file.
...
Follow the below steps to add the Vue Split button component:

1\. First, import and register the Split button component in the `script` section of the **src/App.vue** file.
