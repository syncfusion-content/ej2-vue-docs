...
In this article, the `Material3` theme is applied using CSS styles, which are available in installed packages. The necessary `Material3` CSS styles for the Toolbar component and its dependents were imported into the `<style>` section of **src/App.vue** file.
...
{% endhighlight %}
{% endtabs %}\. In the `template` section, define the Toolbar component with [text](https://ej2.syncfusion.com/vue/documentation/api/toolbar/item#text)
