...
In this article, the `Material3` theme is applied using CSS styles, which are available in installed packages. The necessary `Material3` CSS styles for the Calendar component and its dependencies were imported into the `<style>` section of the **src/App.vue** file.
