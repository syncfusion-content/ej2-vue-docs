...
In this article, `Material` theme is applied using CSS styles, which are available in installed packages. The necessary `Material` CSS styles for the Badge component and its dependencies were imported into the `<style>` section of **src/App.vue** file.
