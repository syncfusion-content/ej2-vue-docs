...
This article uses the [Vue DropDownButton component](https://www.syncfusion.com/vue-components/vue-split-button) as an example. To use the Vue DropDownButton component in the project, the `SplitButton` package needs to be installed using the following command:
...
In this article, `Material3` theme is applied using CSS styles, which are available in installed packages. The necessary `Material3` CSS styles for the Dropdown Button component and its dependents were imported into the `<style>` section of **src/App.vue** file.
...
2.In the `template` section, define the DropDownButton component with the [items](https://ej2.syncfusion.com/vue/documentation/api/drop-down-button/index-default#items) property and content property.
