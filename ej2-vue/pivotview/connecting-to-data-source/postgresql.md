...
2. **Query and Fetch Data**: Execute a SQL query (e.g., `SELECT * FROM table name`) using **NpgsqlCommand** to retrieve data for the Pivot Table.
...
1. In the **App.vue** file, map the Web API URL (`https://localhost:44378/Pivot`) to the Pivot Table using the [url](https://helpej2.syncfusion.com/vue/documentation/api/pivotview/iDataOptions#url) property within the [dataSourceSettings](https://ej2.syncfusion.com/vue/documentation/api/pivotview/dataSourceSettings).
...
1. Configure the Pivot Table report in the **App.vue** file to structure the data retrieved from the PostgreSQL database.
...
Explore a complete example of the Vue Pivot Table integrated with an ASP.NET Core Web Application to fetch data from a PostgreSQL database in this [GitHub](https://github.com/SyncfusionExamples/how-to-bind-PostgreSQL-database-to-pivot-table) repository.