...
description: Rotate, flip, straighten, and zoom the image and its annotations in the Vue Image Editor with configurable min and max zoom levels.
control: Transform
platform: ej2-vue
documentation: ug
domainurl: ##DomainURL##
...
The Image Editor allows you to rotate the image and its annotations by a specific number of degrees clockwise or counter-clockwise using the [rotate](https://ej2.syncfusion.com/vue/documentation/api/image-editor/index-default#rotate) method. The method accepts a single `angle` parameter (`number`, in degrees). Positive values rotate clockwise; negative values rotate counter-clockwise.

`Note:` Pass values in multiples of 90° (e.g., `90`, `180`, `-90`) for proper alignment.

The following example rotates an image in a button click event.
...
The Image Editor provides the [flip](https://ej2.syncfusion.com/vue/documentation/api/image-editor/index-default#flip) method, which allows you to flip both the image and its annotations either horizontally or vertically. This method takes a single parameter of type ImageEditorDirection, which specifies the direction in which the flip operation should be applied.
...
The straightening feature in an Image Editor allows users to adjust an image by rotating it clockwise or counter clockwise. The rotating degree value should be within the range of -45 to +45 degrees for accurate straightening. Positive values indicate clockwise rotation, while negative values indicate counter clockwise rotation. The Image Editor control includes a [straightenImage](https://ej2.syncfusion.com/vue/documentation/api/image-editor/index-default#straightenimage) method, which allows you to adjust the degree of an image. This method takes one parameter that define how the straightening should be carried out:
...
The following example straightens the image.
...
The Image Editor allows you to magnify an image using the [zoom](https://ej2.syncfusion.com/vue/documentation/api/image-editor/index-default#zoom) method. The method allow you to zoom in and out and provides a more detailed view of the image's hidden areas. It takes two parameters:
...
The following example enables panning using the [pan](https://ej2.syncfusion.com/vue/documentation/api/image-editor/index-default#pan) method in a button click event.
...
The [panning](https://ej2.syncfusion.com/vue/documentation/api/image-editor/index-default#panning) event is activated when the user begins dragging the image within the canvas. This event provide an opportunity to perform specific actions, like adjusting the position of an image, in response to the gesture of panning. And these event uses [panEventArgs](https://helpej2.syncfusion.com/vue/documentation/api/image-editor/panEventArgs) to handle the panning action when the user starts dragging the image.

The properties available in the `PanEventArgs` are:
