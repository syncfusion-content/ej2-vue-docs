---
layout: post
title: Virtualization in Vue MultiSelect Dropdown | Syncfusion
description: Render large Syncfusion Vue MultiSelect Dropdown datasets efficiently using enableVirtualization, which reuses DOM nodes and supports incremental search.
...
# Virtualization in Vue MultiSelect Dropdown
...
The MultiSelect component supports the retrieval of data from remote data services with the help of the `DataManager` component, which triggers the `actionBegin` and `actionComplete` events and then updates the list data. During virtual scrolling, additional data is retrieved from the server, triggering the `actionBegin` and `actionComplete` events at that time as well.
...
The following sample shows an example of customizing the item count in virtualization.
...
The following sample shows an example of preselecting values with Virtualization.
